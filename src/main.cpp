...
// Copyright (c) 2018 The Marketcoin developers
...
#if defined(NDEBUG)
# error "Marketcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4a113da30db13c33a64ae7d1941850c2e1d8d9a583f8f83870c2abbe1007d075");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 21); // Marketcoin: difficulty
...
const string strMessageMagic = "Marketcoin Signed Message:\n";
...
{
    // Marketcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight >= 1 && nHeight <= 11)
       nSubsidy = 500000 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Marketcoin: 5 minutes
static const int64 nTargetSpacing = 5 * 60; // Marketcoin: 5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nIntervalLimit = 8 * 60 + 2 * 2;
static const int64 nIntervalMin = 60;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Marketcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if (nHeight <= 11){
            if ((GetBlockTime() - pindexPrev->GetBlockTime()) < nIntervalMin)
                return state.Invalid();
        }else if ((GetBlockTime() - pindexPrev->GetBlockTime()) < nIntervalLimit){
            return state.Invalid();
    	}
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6e8aeef6428adc448a0a0e5c833bf0d0a587af8ed475e7df1850e415654a25b8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Why should we fight to get on top?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1530176967;
        block.nBits    = 0x1e07ffff;
        block.nNonce   = 2093711623;

        if (fTestNet)
        {
            block.nTime    = 1530176983;
            block.nNonce   = 390773381;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcee38220e9a044ac5238e7a0780df905802537ca65ef325366872ee1c08aa24b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb0, 0xd3 }; // Marketcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarketcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarketcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MarketcoinMiner(CWallet *pwallet)
{
    printf("MarketcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("marketcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MarketcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MarketcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MarketcoinMiner, pwallet));
}
